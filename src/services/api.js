...
export async function fetchDataSpecs() {
  ...
  return Promise.all([
    request(`/organizations/${namespace}/requests?state=reviewing&request_type=create_spec`).then(
      data =>
        data.items.map(item => ({
          ...item,
    ...
    request(`/organizations/${namespace}/specs`).then(data =>
      data.items.map(item => ({
        ...item,
